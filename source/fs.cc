...
Value InfoFile(Env env, fs::path filePath, int key)
{
  ...
  fileInfo.Set("createTime", native_fs::ConvertTimeToDate(env, fileStatInfo.st_ctime, _timeZone));
  fileInfo.Set("modifyTime", native_fs::ConvertTimeToDate(env, fileStatInfo.st_mtime, _timeZone));
...
Value InfoFolderFromPath(const CallbackInfo &info)
{
  ...
  std::string arg0 = info[0].As<String>().ToString();
  fs::path filePath = fs::path(arg0);

  _timeZone = info[1].As<Number>().ToNumber();
  ...
  try
  {
    if (fs::is_directory(filePath))
    {
      ...
      {
        auto data = InfoFile(env, p.path(), index);
        if (data != env.Null())
        {
          arr.Set(index, data);
          index++;
...
Object Init(Env env, Object exports)
{
  ...
  exports.Set(String::New(env, "isFolder"),
              Function::New(env, CheckOnFolder));
